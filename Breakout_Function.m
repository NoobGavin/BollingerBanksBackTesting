function [record,Summary]=Breakoutfunction(Price,Dateee,n,UpperFactor,LowerFactor)
% Break out trading strategy and calculate trading table
N=length(Price);
mu=zeros(N-n+1,1);
...
if Price(N-n+1)>Upperband(N-n+1)
    ...
    tmp=[N-n+1,Dateee(N-n+1),Price(N-n+1),tm,tm,Status,size,tmm];
    record=[record;tmp];
